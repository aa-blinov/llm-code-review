...
def main():
    ...
    logger.info("🚀 Запуск анализа merge request...")
    ...
    if "github.com" in args.merge_request_url:
        logger.info("🔍 Определен провайдер: GitHub")
        provider = GitHubProvider()
    elif "gitlab.com" in args.merge_request_url:
        logger.info("🔍 Определен провайдер: GitLab")
        provider = GitLabProvider()
    else:
        logger.error("❌ Неподдерживаемый URL merge request")
    ...
    logger.info("📥 Получение данных merge request...")
    ...
    logger.info(f"✅ Данные получены: '{merge_request_data.get('title', 'Unknown')}'")
    ...
    if ai_sections:
        ...
        logger.info("ℹ️  AI анализ пропущен или не дал результатов")
    ...
    logger.info(f"✅ Отчет сохранен: {out_path}")
